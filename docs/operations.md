# CRM System — One Stop Shop Portal operations
...
Windows can run the binary-safe equivalent with `powershell -ExecutionPolicy Bypass -File deploy/rehearse-backup-restore.ps1`. Use an isolated rehearsal environment and verify the health endpoint before restoring production traffic.
...
2. Run `powershell -File deploy/setup-production.ps1` to generate `.env.production` with fresh database/JWT/admin secrets, then fill in the Brevo and domain values it leaves as placeholders.
...
The supplied units assume the standard deployment location `/opt/crm-system`, with the current release at `/opt/crm-system/current` and the protected environment file at `/opt/crm-system/shared/.env.production`. If `DEPLOY_PATH` differs, update those paths before installation.

```sh
sudo install -m 0644 deploy/systemd/*.service deploy/systemd/*.timer /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now crm-system.service
sudo systemctl enable --now crm-system-backup.timer crm-system-certbot-renew.timer
systemctl list-timers 'crm-system-*'
```

`crm-system-certbot-renew.timer` checks certificates twice daily and reloads Nginx after a successful renewal command. Inspect operations with `journalctl -u crm-system-backup.service` and `journalctl -u crm-system-certbot-renew.service`.
