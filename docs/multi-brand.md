...
CRM System — One Stop Shop Portal is one reusable codebase for BAU, DIGITMAK, VEZILKA and HPC. The runtime supports one API and one shared database, with a central platform view that can report which tenant uses which services. Tenant context is resolved from the verified request host, not from a client-supplied tenant id. Brand identity remains configuration/data; business workflows must not be copied or forked per organisation.
...
Production requests are mapped to tenants from `Tenancy:Hosts:{tenantId}`. Supported default hosts are `portal.{tenant}.mk`, `crm.{tenant}.mk`, plus the public base domain for each configured centre. Unknown production hosts are rejected. The compiled frontend prefers the browser hostname over `VITE_BRAND_ID`, so the same frontend image can serve multiple brand domains.

`CreatedTenantId` and `OwnerTenantId` remain the real CRM ownership boundary. `SourceTenantId` is attribution-only and is populated server-side from a verified affiliate code; it never changes ownership.
