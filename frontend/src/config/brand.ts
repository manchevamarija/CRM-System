...
const brandFromHost = (host: string): BrandId => {
  ...
  // Brand is decided by BRAND_ID (below), not by hostname — this deployment serves
  // generic "CRM System" branding from crm.digitmak.mk. Affiliate brands are still
  // selected by path (/bau/<partner>, /hpc/<partner>, ...), not by hostname.
...
const hostId = brandFromHost(
  typeof window === "undefined" ? "" : window.location.hostname,
);
const resolvedId = hostId !== "crm" ? hostId : (isBrandId(configuredId) ? configuredId : "crm");
