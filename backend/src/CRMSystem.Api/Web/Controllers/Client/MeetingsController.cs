...
public sealed class MeetingsController(IMeetingService service, PortalDbContext db) : ControllerBase
{
    ...
    public async Task<object> Mine(CancellationToken ct)
    {
        ...
        var names = await db.Users.Where(user => assignedIds.Contains(user.Id)).ToDictionaryAsync(user => user.Id, user => $"{user.FirstName} {user.LastName}".Trim() == "" ? user.Email ?? "One Stop Shop Portal" : $"{user.FirstName} {user.LastName}".Trim(), ct);
