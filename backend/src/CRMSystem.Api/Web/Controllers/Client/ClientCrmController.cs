...
public sealed class ClientCrmController(PortalDbContext db, IHubContext<CrmHub> crmHub) : ControllerBase
{
    ...
    public async Task<IResult> AddService(Guid id, AddServiceRequest request, CancellationToken ct)
    {
        ...
        var (serviceAddedSubject, serviceAddedBody) = item.PreferredLanguage switch
        {
            "en" => ($"Service added: {name}", $"<p>You successfully added the service <strong>{encodedName}</strong> to your request.</p>"),
            "sq" => ($"Shërbimi u shtua: {name}", $"<p>E shtuat me sukses shërbimin <strong>{encodedName}</strong> në kërkesën tuaj.</p>"),
            _ => ($"Додадена услуга: {name}", $"<p>Успешно ja додадовте услугата <strong>{encodedName}</strong> во вашето барање.</p>"),
        };
        db.Notifications.Add(new Notification
        {
            RecipientUserId = userId,
            Language = item.PreferredLanguage,
            Type = "CrmServiceAddedConfirmation",
            Subject = serviceAddedSubject,
            Body = serviceAddedBody,
