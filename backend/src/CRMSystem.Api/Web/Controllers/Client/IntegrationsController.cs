    ...
    : ControllerBase
{
    ...
    public async Task<IResult> MoodleLaunch()
    {
        ...
        var url =
            $"{baseUrl}{separator}crm_system_user={Uri.EscapeDataString(user.Id.ToString())}&email={Uri.EscapeDataString(user.Email ?? "")}&name={Uri.EscapeDataString($"{user.FirstName} {user.LastName}")}&lang={Uri.EscapeDataString(user.PreferredLanguage)}&ts={timestamp}&signature={signature}";
