...
namespace CRMSystem.Api.Web.Controllers.Files;
...
public sealed class FilesController(IFileStorage storage, PortalDbContext db) : ControllerBase
{
    ...
    public async Task<IResult> Delete(Guid id, CancellationToken ct)
    {
        ...
        var contactAttachment = await db.ContactRequestAttachments.SingleOrDefaultAsync(x => x.FileId == id, ct);
        if (contactAttachment is not null)
            db.ContactRequestAttachments.Remove(contactAttachment);
