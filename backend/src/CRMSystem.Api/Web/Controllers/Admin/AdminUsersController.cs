...
public sealed class AdminUsersController(
    ...
    : ControllerBase
{
    // Any Admin (whether scoped to one brand or not) can manage the users visible to
    // them — the tenant-isolation query filter on UserTenantMemberships already
    // restricts a brand-scoped admin to just their own brand's users, so this check
    // only needs to exclude non-admin roles entirely, not further exclude brand-scoped
    // admins from managing their own brand's users.
    private bool CanManageScopedUsers => User.IsInRole(PortalRoles.PlatformAdmin)
        || User.IsInRole(PortalRoles.Admin);
    ...
    public async Task<IResult> CreateUser(CreateUserRequest request, CancellationToken ct)
    {
        ...
        db.Notifications.Add(AdminUserWelcomeEmail.Create(user, activationUrl, tenantContext.Current.Name));
        db.AuditLogs.Add(Audit(User, "UserCreated", nameof(AppUser), user.Id, null, role));
        // A client linked straight to an organisation here (skipping the usual contact
        // request → services → invite flow) still needs a subscription to get portal
        // access — send that invitation now so they aren't left wondering why they can't
        // do anything after activating, or forgotten about entirely.
        if (role.Equals(PortalRoles.Client, StringComparison.OrdinalIgnoreCase)
            && request.OrganizationId is { } organizationId
            && await db.Organizations.AnyAsync(o => o.Id == organizationId && o.Status == "Approved", ct))
        {
            var raw = Convert
                .ToBase64String(RandomNumberGenerator.GetBytes(36))
                .Replace("/", "_")
                .Replace("+", "-");
            var invitation = new SubscriptionInvitation
            {
                UserId = user.Id,
            ...
            db.Add(invitation);
            var inviteLink = $"{publicUrl.TrimEnd('/')}/subscription-invite?token={Uri.EscapeDataString(raw)}";
            db.Notifications.Add(
                new Notification
                {
                    ...
                    Body =
                        $"<p>Вашата годишна претплата е подготвена.</p><p><a href=\"{inviteLink}\">Прифати ja поканата</a></p>",
                }
            );
        }
