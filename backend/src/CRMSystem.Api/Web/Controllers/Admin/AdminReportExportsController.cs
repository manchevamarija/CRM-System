...
public sealed class AdminReportExportsController(PortalDbContext db) : ControllerBase
{
            ...
            rows.Select(x =>
                    (IReadOnlyList<string?>)
                        [
                            ...
                            x.CreatedAt.ToSkopje().ToString("dd.MM.yyyy HH:mm"),
                        ]
                )
                .ToList(),
    ...
    public async Task<IResult> ExportDataset(string dataset, string? format, CancellationToken ct)
    {
        ...
        {
            case "contacts":
                title = "CRM System — One Stop Shop Portal · Контакт-барања";
                headers =
                [
                    "Организација",
                    "Тип на организација",
                ...
                break;
            case "meetings":
                title = "CRM System — One Stop Shop Portal · Состаноци";
                headers =
                [
                    "Наслов",
                    "Тип",
                    ...
                    .Select(x =>
                        (IReadOnlyList<string?>)
                            [
                                ...
                                x.StartsAt?.ToSkopje().ToString("dd.MM.yyyy HH:mm"),
                                x.CreatedAt.ToSkopje().ToString("dd.MM.yyyy HH:mm"),
                            ]
                    )
                    .ToList();
                break;
            case "subscriptions":
                title = "CRM System — One Stop Shop Portal · Претплати";
                    ...
                    .Select(x =>
                        (IReadOnlyList<string?>)
                            [
                                ...
                                Label(x.Status),
                                x.StartsAt?.ToSkopje().ToString("dd.MM.yyyy HH:mm"),
                                x.ExpiresAt?.ToSkopje().ToString("dd.MM.yyyy HH:mm"),
                                x.CreatedAt.ToSkopje().ToString("dd.MM.yyyy HH:mm"),
                            ]
                    )
                    .ToList();
                break;
            case "tickets":
                title = "CRM System — One Stop Shop Portal · Тикети";
                headers =
                [
                    "Број на тикет",
                    "Категорија",
                ...
                break;
            case "crm-demand":
                title = "CRM System — One Stop Shop Portal · Побарувачка по услуги и клиенти";
                headers = ["Клиент", "Е-пошта", "Организација", "Услуга", "CRM статус", "Affiliate", "Партнер", "Source tenant", "Креирано"];
                var demand = await db.ContactRequests.OrderBy(x => x.CreatedAt).ToListAsync(ct);
                rows = demand.SelectMany(x => { try { return System.Text.Json.JsonSerializer.Deserialize<List<ServiceName>>(x.ServiceItemsJson, new System.Text.Json.JsonSerializerOptions(System.Text.Json.JsonSerializerDefaults.Web)) ?? []; } catch { return []; } }, (request, service) => (IReadOnlyList<string?>)[request.ContactName, request.Email, request.OrganizationName, service.Name, Label(request.Status), request.AffiliateCode ?? "", request.PartnerCode ?? "", request.SourceTenantId ?? "", request.CreatedAt.ToSkopje().ToString("dd.MM.yyyy HH:mm")]).ToList();
                break;
            case "crm-analytics":
                title = "CRM System — One Stop Shop Portal · Оперативна CRM аналитика";
                headers = ["Референца", "Клиент", "Организација", "Тип барање", "CRM статус", "Affiliate", "Партнер", "Source tenant", "Услуга", "Статус услуга", "Цена", "Рок", "Одговорен агент", "Буџет", "Креирано"];
                var analytics = await db.ContactRequests.OrderBy(x => x.CreatedAt).ToListAsync(ct);
                rows = analytics.SelectMany(x => { try { return System.Text.Json.JsonSerializer.Deserialize<List<AnalyticsService>>(x.ServiceItemsJson, new System.Text.Json.JsonSerializerOptions(System.Text.Json.JsonSerializerDefaults.Web)) ?? []; } catch { return []; } }, (request, service) => (IReadOnlyList<string?>)[$"CRM-{request.Id.ToString("N")[..8].ToUpperInvariant()}", request.ContactName, request.OrganizationName, Label(request.RequestType), Label(request.Status), request.AffiliateCode ?? "", request.PartnerCode ?? "", request.SourceTenantId ?? "", service.Name, Label(service.Status), service.Price?.ToString("0.00"), service.Deadline?.ToSkopje().ToString("dd.MM.yyyy"), UserName(users, service.AssignedAgentId), request.BudgetRange, request.CreatedAt.ToSkopje().ToString("dd.MM.yyyy HH:mm")]).ToList();
                break;
            default:
