    ...
    : ControllerBase
{
    ...
    public async Task<IResult> InviteRegistration(Guid id)
    {
        ...
        var (subject, greeting, intro, cta, closing) = language switch
        {
            ...
            _ => (
                ...
                "Овoj линк е личен — ве молиме не го препраќajте."
            ),
        };
        db.Notifications.Add(
            new Notification
    ...
    private static Notification StatusEmail(ContactRequest item, string status)
    {
        ...
        {
            "en" => isHandled
                ? "Your One Stop Shop Portal contact request has been processed"
                : "Your One Stop Shop Portal contact request is being processed",
            "sq" => isHandled
                ? "Kërkesa juaj e kontaktit në One Stop Shop Portal është përpunuar"
                : "Kërkesa juaj e kontaktit në One Stop Shop Portal është në përpunim",
            _ => isHandled
                ? "Вашето контакт барање до One Stop Shop Portal е обработено"
                : "Вашето контакт барање до One Stop Shop Portal е во обработка",
        };
        var body = language switch
        {
            "en" => isHandled
                ...
                : "<p>Status: <strong>In processing</strong>.</p><p>A member of the One Stop Shop Portal team has started reviewing your contact request.</p>",
            "sq" => isHandled
                ...
                : "<p>Statusi: <strong>Në përpunim</strong>.</p><p>Një anëtar i ekipit One Stop Shop Portal ka filluar shqyrtimin e kërkesës suaj.</p>",
            _ => isHandled
                ...
                : "<p>Статус: <strong>Во обработка</strong>.</p><p>Член од тимот на One Stop Shop Portal започна со разгледување на вашето контакт барање.</p>",
        };
    ...
    private static string ResponseSubject(string? language) =>
        ...
        {
            "en" => "One Stop Shop Portal response to your contact request",
            "sq" => "Përgjigje nga One Stop Shop Portal për kërkesën tuaj",
            _ => "Одговор од One Stop Shop Portal за вашето контакт барање",
        };
