...
public sealed class AdminOrganizationsController(PortalDbContext db) : ControllerBase
{
    ...
    public sealed record SetOrganizationBrandRequest(string Brand);
    ...
    [HttpPut("organizations/{id:guid}/brand")]
    public async Task<IResult> SetBrand(Guid id, SetOrganizationBrandRequest request, CancellationToken ct)
    {
        var item = await db.Organizations.FirstOrDefaultAsync(x => x.Id == id, ct);
        if (item is null) return Results.NotFound();
        var validBrands = new[] { "digitmak", "vezilka", "hpc", "bau" };
        if (!validBrands.Contains(request.Brand))
            return Results.BadRequest(new { message = "Unknown brand." });
        item.Brand = request.Brand;
        db.AuditLogs.Add(Audit(User, "OrganizationBrandChanged", nameof(Organization), item.Id));
        await db.SaveChangesAsync(ct);
        return Results.Ok(item);
    }
    ...
    public async Task<IResult> ChangeStatus(Guid id, string operation)
    {
        ...
        var (orgSubject, orgBody) = next switch
        {
            "Approved" => (
                ...
                $"<p>Организацијата <strong>{item.Name}</strong> е одобрена. Можете да продолжете со претплата.</p>"
            ),
            "Rejected" => (
    ...
    public async Task<IResult> TransferClient(TransferClientRequest request, CancellationToken ct)
    {
        ...
        // The client's own UserTenantMembership decides which brand's admin panel can
        // even see them in the Users list — moving their organization without also
        // moving this made them invisible to the new brand's admins (they'd still only
        // show up under whichever brand they originally registered through).
        var membership = await db.UserTenantMemberships
            .IgnoreQueryFilters()
            .FirstOrDefaultAsync(x => x.UserId == client.Id, ct);
        if (membership is not null && !string.IsNullOrWhiteSpace(toOrganization.Brand))
            membership.TenantId = toOrganization.Brand;
