...
) : ControllerBase
{
    ...
    public async Task<IResult> Assign(Guid id, TicketAssignmentRequest request)
    {
        ...
        if (!db.IsGlobalScope)
        {
            var assignees = new[] { request.AgentId, request.ExpertId }
                .Where(x => x is not null).Select(x => x!.Value).Distinct().ToArray();
            if (assignees.Length > 0 && await db.Users
        ...
        db.Notifications.Add(
            new Notification
            {
                ...
                Body = $"<p>Вашиот тикет е доделен на тимот на {tenantContext.Current.Name}.</p>",
