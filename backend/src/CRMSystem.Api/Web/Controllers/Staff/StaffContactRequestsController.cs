...
public sealed class StaffContactRequestsController(PortalDbContext db) : ControllerBase
{
    ...
    public async Task<IResult> UpdateWorkflow(Guid id, WorkflowRequest request, CancellationToken ct)
    {
        ...
        var (statusSubject, statusBody) = item.PreferredLanguage switch
        {
            "en" => ($"CRM status: {item.Status}", $"<p>Your request has been updated. New status: <strong>{item.Status}</strong>.</p>"),
            "sq" => ($"Statusi CRM: {item.Status}", $"<p>Kërkesa juaj u përditësua. Statusi i ri: <strong>{item.Status}</strong>.</p>"),
            _ => ($"CRM статус: {item.Status}", $"<p>Вашето барање е ажурирано. Нов статус: <strong>{item.Status}</strong>.</p>"),
        };
        db.Notifications.Add(new Notification { RecipientUserId = item.UserId, RecipientEmail = item.Email, Language = item.PreferredLanguage, Type = "CrmStatusChanged", Subject = statusSubject, Body = statusBody, ActionUrl = "/portal?tab=crm" });
