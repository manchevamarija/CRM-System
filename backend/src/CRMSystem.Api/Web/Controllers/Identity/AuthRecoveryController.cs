...
public sealed partial class AuthController
{
    ...
    public async Task<IResult> ForgotPassword(EmailRequest request)
    {
        ...
        if (user is not null)
        {
            ...
            var (resetSubject, resetBody) = user.PreferredLanguage switch
            {
                "en" => (
                    $"{tenantContext.Current.Name} password reset",
                    $"<p>Reset your password:</p><p><a href=\"{WebUtility.HtmlEncode(link)}\">Set a new password</a></p>"
                ),
                "sq" => (
                    $"Rivendosja e fjalëkalimit në {tenantContext.Current.Name}",
                    $"<p>Rivendosni fjalëkalimin tuaj:</p><p><a href=\"{WebUtility.HtmlEncode(link)}\">Vendosni fjalëkalim të ri</a></p>"
                ),
                _ => (
                    $"Обновување лозинка за {tenantContext.Current.Name}",
                    $"<p>Обновете ja вашата лозинка:</p><p><a href=\"{WebUtility.HtmlEncode(link)}\">Постави нова лозинка</a></p>"
                ),
            };
    ...
    private void SetRefreshCookie(HttpResponse response, string token, IHostEnvironment env) =>
        response.Cookies.Append(
            ...
            {
                HttpOnly = true,
                // Base this on whether THIS request actually arrived over HTTPS (accounting
                // for a reverse proxy in front, via ForwardedHeaders) rather than hardcoding
                // "always true in production" — that broke session persistence entirely when
                // testing directly over plain HTTP (e.g. the bare server IP before DNS/TLS was
                // live), since browsers silently refuse to store a Secure cookie over HTTP.
