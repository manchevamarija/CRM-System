...
public sealed partial class AuthController : ControllerBase
{
    ...
    public async Task<IResult> Register(RegisterRequest request)
    {
        ...
        // This deployment doesn't require an email-confirmation step for self-service
        // registration — new accounts go straight to Active.
        var developmentVerified = true;
        ...
        foreach (var item in matchingRequests)
            item.UserId = user.Id;
        // Emails sent before this account existed (the initial confirmation, a
        // registration invite) only had RecipientEmail set — back-fill RecipientUserId
        // now so they also show up in this person's own in-app notification bell,
        // not just their inbox.
        await db.Notifications
            .Where(item => item.RecipientUserId == null && item.RecipientEmail != null && item.RecipientEmail.ToLower() == request.Email.ToLower())
            .ExecuteUpdateAsync(setters => setters.SetProperty(item => item.RecipientUserId, user.Id));
        ...
        if (!developmentVerified)
        {
            ...
            var brandName = tenantContext.Current.Name;
            var (verifySubject, verifyBody) = user.PreferredLanguage switch
            {
                "en" => (
                ...
                "sq" => (
                    $"Verifikoni llogarinë tuaj në {brandName}",
                    $"<p>Konfirmoni llogarinë tuaj:</p><p><a href=\"{WebUtility.HtmlEncode(link)}\">Verifiko email-in</a></p>"
                ),
                _ => (
                    $"Потврдете ja вашата {brandName} сметка",
                    $"<p>Потврдете ja вашата сметка:</p><p><a href=\"{WebUtility.HtmlEncode(link)}\">Потврди е-пошта</a></p>"
                ),
            };
