    ...
    )
    {
        ...
        services.AddDbContext<PortalDbContext>(options =>
        {
            if (!string.IsNullOrWhiteSpace(cs))
            {
                ...
                // TEMPORARY: the EF Core model snapshot has a residual mismatch versus
                // the current model that migrations alone haven't fully resolved yet
                // (tracked for a proper fix). Suppressing this specific warning lets the
                // app start against PostgreSQL in the meantime; it does not affect data
                // safety — it only silences a startup-time consistency check.
                    ...
                    OnMessageReceived = c =>
                    {
                        if (
                            ...
                            && (c.HttpContext.Request.Path.StartsWithSegments("/hubs")
                                || c.HttpContext.Request.Path.StartsWithSegments("/api/staff/calendar/connect"))
                        )
                            c.Token = token;
        ...
        services.AddScoped<CalendarTokenStore>();
