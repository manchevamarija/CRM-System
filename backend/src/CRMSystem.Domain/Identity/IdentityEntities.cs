...
public class AppUser : IdentityUser<Guid>
{
    ...
    /// <summary>Restricts a staff/admin account to seeing only leads/requests attributed
    /// to this one brand (e.g. "bau") — null/empty means no restriction (sees every
    /// brand, the default for existing accounts). Set from the Users admin panel.</summary>
    public string? AssignedBrand { get; set; }
    /// <summary>Optional partner-level scope under AssignedBrand. When set, the account only sees records attributable to that partner.</summary>
