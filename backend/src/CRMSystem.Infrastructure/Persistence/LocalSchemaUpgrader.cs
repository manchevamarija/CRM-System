...
{
    private const int CurrentLocalSchemaVersion = 8;
    ...
    private static async Task EnsureLocalSchemaAsync(PortalDbContext db)
    {
        ...
        await AddColumnIfMissingAsync(db, "ContactRequests", "SourceTenantId", "TEXT NULL");
        await AddColumnIfMissingAsync(db, "ContactRequests", "AffiliateCode", "TEXT NULL");
        await AddColumnIfMissingAsync(db, "ContactRequests", "PartnerCode", "TEXT NULL");
        await AddColumnIfMissingAsync(db, "ContactRequests", "AffiliateClickId", "TEXT NULL");
        await AddColumnIfMissingAsync(db, "ContactRequests", "AttributionStatus", "TEXT NOT NULL DEFAULT 'Unattributed'");
        await AddColumnIfMissingAsync(db, "ContactRequests", "AttributionReason", "TEXT NULL");
        await AddColumnIfMissingAsync(db, "ContactRequests", "GoogleFormResponseId", "TEXT NULL");
        await db.Database.ExecuteSqlRawAsync(
            """
            CREATE INDEX IF NOT EXISTS "IX_ContactRequests_AffiliateCode"
                ON "ContactRequests" ("AffiliateCode");
            CREATE INDEX IF NOT EXISTS "IX_ContactRequests_AffiliateCode_PartnerCode"
                ON "ContactRequests" ("AffiliateCode", "PartnerCode");
            CREATE UNIQUE INDEX IF NOT EXISTS "IX_ContactRequests_AffiliateClickId"
                ON "ContactRequests" ("AffiliateClickId") WHERE "AffiliateClickId" IS NOT NULL;
            CREATE UNIQUE INDEX IF NOT EXISTS "IX_ContactRequests_GoogleFormResponseId"
                ON "ContactRequests" ("GoogleFormResponseId");
            ...
            CREATE INDEX IF NOT EXISTS "IX_AffiliateClicks_TenantId_AffiliateCode_CreatedAt"
                ON "AffiliateClicks" ("TenantId", "AffiliateCode", "CreatedAt");
            CREATE INDEX IF NOT EXISTS "IX_AffiliateClicks_AffiliateCode_PartnerCode"
                ON "AffiliateClicks" ("AffiliateCode", "PartnerCode");
            CREATE UNIQUE INDEX IF NOT EXISTS "IX_AffiliateClicks_ClickId"
                ON "AffiliateClicks" ("ClickId");
            CREATE TABLE IF NOT EXISTS "AffiliatePartners" (
                "Id" TEXT NOT NULL CONSTRAINT "PK_AffiliatePartners" PRIMARY KEY,
            ...
            CREATE UNIQUE INDEX IF NOT EXISTS "IX_AffiliatePartners_BrandCode_PartnerCode"
                ON "AffiliatePartners" ("BrandCode", "PartnerCode");
            """
        );
        await db.Database.ExecuteSqlRawAsync(
            """
        ...
        await AddColumnIfMissingAsync(db, "ContactRequestTransfers", "ApprovalStatus", "TEXT NOT NULL DEFAULT 'Approved'");
        await AddColumnIfMissingAsync(db, "ContactRequestTransfers", "SourceApprovedBy", "TEXT NULL");
        await AddColumnIfMissingAsync(db, "ContactRequestTransfers", "SourceApprovedAt", "INTEGER NULL");
        await AddColumnIfMissingAsync(db, "ContactRequestTransfers", "DestinationApprovedBy", "TEXT NULL");
        await AddColumnIfMissingAsync(db, "ContactRequestTransfers", "DestinationApprovedAt", "INTEGER NULL");
        await AddColumnIfMissingAsync(db, "ContactRequestTransfers", "CompletedAt", "INTEGER NULL");
        await db.Database.ExecuteSqlRawAsync("CREATE INDEX IF NOT EXISTS \"IX_ContactRequestTransfers_ContactRequestId_ApprovalStatus\" ON \"ContactRequestTransfers\" (\"ContactRequestId\", \"ApprovalStatus\");");
        await db.Database.ExecuteSqlRawAsync(
            """
...
#pragma warning restore EF1002
    ...
    )
    {
        ...
        var organization = await db.Organizations.SingleOrDefaultAsync(x =>
            x.CreatedByUserId == user.Id && x.Name == "CRM System — One Stop Shop Portal Demo Organization"
        );
        if (organization is null)
        {
            organization = new Organization
            {
                Name = "CRM System — One Stop Shop Portal Demo Organization",
                Type = "SME",
