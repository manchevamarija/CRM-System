...
{
    Task<IReadOnlyList<Guid>> GetAdminUserIdsAsync(string brandCode, CancellationToken cancellationToken);
    Task<(Guid? UserId, Guid? OrganizationId)> ResolveAssociationAsync(string email, string organizationName, CancellationToken cancellationToken);
    Task<bool> AffiliateClickExistsAsync(string clickId, string affiliateCode, string? partnerCode, CancellationToken cancellationToken);
    Task<ContactRequest?> FindByGoogleFormResponseIdAsync(string responseId, CancellationToken cancellationToken);
    Task SetGoogleFormResponseIdAsync(Guid contactRequestId, string responseId, CancellationToken cancellationToken);
    Task<Guid?> FindPartnerLinkedOrganizationIdAsync(string brandCode, string? partnerCode, CancellationToken cancellationToken);
