...
public static partial class DatabaseInitializer
{
    public static async Task InitializePortalAsync(this IServiceProvider services)
    {
        ...
        await EnsureDefaultAffiliatePartnersAsync(db);
    ...
    private static async Task EnsureDefaultAffiliatePartnersAsync(PortalDbContext db)
    {
        var defaultPartners = Enumerable.Range(1, 8)
            .Select(index => $"partner-{index:00}")
            .ToArray();

        var existing = await db.AffiliatePartners
            .IgnoreQueryFilters()
            .Where(x => x.BrandCode == "bau" && defaultPartners.Contains(x.PartnerCode))
            .Select(x => x.PartnerCode)
            .ToListAsync();

        foreach (var code in defaultPartners.Where(code => !existing.Contains(code)))
        {
            db.AffiliatePartners.Add(new AffiliatePartner
            {
                BrandCode = "bau",
                PartnerCode = code,
                Name = $"BAU Partner {code[^2..]}",
                IsActive = true,
            });
        }

        if (defaultPartners.Any(code => !existing.Contains(code)))
            await db.SaveChangesAsync();
    }
