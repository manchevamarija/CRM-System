...
internal static class ContactRequestPdfRenderer
{
    ...
    private static void ComposeHeader(PageDescriptor page)
    {
        page.Header().Column(header =>
        {
            header.Item().Row(row =>
            {
                row.ConstantItem(58).Height(58).Svg(CrmLogoIconSvg);
                row.RelativeItem().PaddingLeft(12).PaddingTop(3).Column(brand =>
                {
                    brand.Item().Text("CRM System").FontSize(19).Bold().FontColor(PdfNavy);
                    brand.Item().PaddingTop(2).Text("DIGITAL SERVICE PLATFORM")
                        .FontSize(8).SemiBold().FontColor(PdfMuted);
                });
            });
        ...
        string language)
    {
        page.Content().PaddingTop(14).Column(content =>
        {
            ...
            ComposeNextSteps(content, item.Status, language);
        ...
        string language)
    {
        ...
        content.Item().Table(table =>
        {
            ...
            table.Cell().Element(InfoCell).Element(cell => InfoValue(cell,
                Text(language, "ПОДНЕСЕНО", "SUBMITTED", "DATA E PARAQITJES"), $"{item.CreatedAt.ToSkopje():dd.MM.yyyy} {item.CreatedAt.ToSkopje():HH:mm}"));
        });
    }
    ...
    private static void ComposeHandover(ColumnDescriptor content, HandoverContext handover, string language)
    {
        ...
        content.Item().Table(table =>
        {
            ...
            table.Cell().ColumnSpan(2).Element(InfoCell).Element(cell => InfoValue(cell,
                Text(language, "ДАТУМ НА ПРИМОПРЕДАВАЊЕ", "HANDOVER DATE", "DATA E BARTJES"), $"{handover.TransferredAt.ToSkopje():dd.MM.yyyy} {handover.TransferredAt.ToSkopje():HH:mm}"));
        });
    ...
    private static void ComposeServices(ColumnDescriptor content, string[] services, string language)
    {
        ...
        for (var i = 0; i < services.Length; i++)
        {
            ...
            {
                text.Span($"{i + 1}.  ").Bold().FontSize(12).FontColor(PdfTeal);
                text.Span(LocalizeService(service, language));
            });
        }
    }

    private static void ComposeNextSteps(ColumnDescriptor content, string status, string language)
    {
        // Once the request has moved past the very first stage (contact already made,
        // service already confirmed/priced, etc.), telling the client "our team will
        // contact you" is stale and misleading — only show it while still freshly
        // submitted and awaiting that first contact.
        if (status != "Applied")
            return;
    ...
    private const string CrmLogoIconSvg = """
        <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64" role="img" aria-label="CRM System">
          <defs>
            <linearGradient id="g" x1="7" y1="7" x2="57" y2="57" gradientUnits="userSpaceOnUse">
              <stop stop-color="#173F5F"/><stop offset="1" stop-color="#267A82"/>
            </linearGradient>
          </defs>
          <g>
            <path d="M32 3 57 17.5v29L32 61 7 46.5v-29Z" fill="url(#g)"/>
            <path d="M22 20.5h16.5l5 5v13l-5 5H22l-5-5v-13Z" fill="none" stroke="#fff" stroke-width="3" stroke-linejoin="round"/>
            <path d="M27 27h10v10H27z" fill="none" stroke="#D9AB55" stroke-width="2.5"/>
            <path d="M17 27h-5m5 10h-5m35-10h5m-5 10h5M27 20v-5m10 5v-5m-10 29v5m10-5v5" stroke="#D9AB55" stroke-width="2" stroke-linecap="round"/>
            <circle cx="11" cy="27" r="2" fill="#fff"/><circle cx="11" cy="37" r="2" fill="#fff"/><circle cx="53" cy="27" r="2" fill="#fff"/><circle cx="53" cy="37" r="2" fill="#fff"/>
          </g>
        </svg>
        """;
}
