    ...
    : ITokenService
{
    public async Task<string> CreateAsync(AppUser user)
    {
        ...
        if (!string.IsNullOrWhiteSpace(user.AssignedBrand))
            claims.Add(new Claim("crm:assigned-brand", user.AssignedBrand.Trim().ToLowerInvariant()));
        if (!string.IsNullOrWhiteSpace(user.AssignedPartnerCode))
            claims.Add(new Claim("crm:assigned-partner", user.AssignedPartnerCode.Trim().ToLowerInvariant()));
        if (roles.Contains(PortalRoles.PlatformAdmin, StringComparer.OrdinalIgnoreCase))
        {
            claims.Add(new Claim("crm:global-scope", "true"));
            claims.Add(new Claim("crm:scope-kind", "platform-admin"));
        }
        else if (roles.Contains(PortalRoles.Admin, StringComparer.OrdinalIgnoreCase))
        {
            var scopeKind = string.IsNullOrWhiteSpace(user.AssignedBrand)
                ? "global-admin"
                : string.IsNullOrWhiteSpace(user.AssignedPartnerCode)
                    ? "brand-admin"
                    : "partner-admin";
            if (scopeKind == "global-admin")
                claims.Add(new Claim("crm:global-scope", "true"));
            claims.Add(new Claim("crm:scope-kind", scopeKind));
        }
