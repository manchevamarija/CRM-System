...
param(
    ...
    [string]$ProjectName = 'crm_system_rehearsal',
...
if ($ProjectName -notmatch '^crm_system_[a-z0-9_-]+_rehearsal$') { throw 'ProjectName must identify an isolated CRM System rehearsal project.' }
...
$stamp = [DateTime]::UtcNow.ToString('yyyyMMddTHHmmssZ'); $dump = Join-Path $databaseDir "crm-system-$stamp.dump"; $archive = Join-Path $uploadsDir "uploads-$stamp.tar.gz"
...
Invoke-DockerBinary "$compose api tar -C /var/lib/crm-system/uploads -czf - ." $archive
...
Invoke-DockerBinary "$compose api tar -C /var/lib/crm-system/uploads -xzf -" '' $archive
