...
try {
    ...
    if (-not $SkipSourceVerification) {
        Invoke-Step 'Backend Release build' {
            dotnet build CRMSystem.slnx -c Release --nologo
        }
        Invoke-Step 'Frontend install' {
            Push-Location frontend
            try { npm.cmd ci } finally { Pop-Location }
        }
        Invoke-Step 'Frontend lint and build' {
            Push-Location frontend
            try {
                npm.cmd run lint
    ...
    if ($Mode -eq 'Deploy') {
        ...
        if (-not $SkipBackupRehearsal) {
            Invoke-Step 'Backup and restore rehearsal' {
                & "$PSScriptRoot/rehearse-backup-restore.ps1" -EnvFile $EnvFile -ProjectName 'crm_system_production_rehearsal'
            }
        }
