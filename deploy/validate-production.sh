...
[ -f "$ENV_FILE" ] || { echo "Missing $ENV_FILE. Run: powershell -File deploy/setup-production.ps1" >&2; exit 1; }

env_value() { sed -n "s/^$1=//p" "$ENV_FILE" | tail -n 1; }
required='POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD JWT_SIGNING_KEY BREVO_SMTP_HOST BREVO_SMTP_PORT BREVO_SMTP_USERNAME BREVO_SMTP_PASSWORD BREVO_FROM_EMAIL ADMIN_BOOTSTRAP_EMAIL ADMIN_BOOTSTRAP_PASSWORD UPLOADS_ROOT CLAMAV_HOST CLAMAV_PORT LETSENCRYPT_EMAIL ALLOWED_HOSTS CORS_ALLOWED_ORIGINS_0 PORTAL_DOMAIN PORTAL_DOMAINS TLS_PRIMARY_DOMAIN APP_PUBLIC_URL GOOGLE_FORM_WEBHOOK_SECRET'
...
PORTAL_DOMAINS="$(env_value PORTAL_DOMAINS)"
TLS_PRIMARY_DOMAIN="$(env_value TLS_PRIMARY_DOMAIN)"
...
[ "$TLS_PRIMARY_DOMAIN" = "$PORTAL_DOMAIN" ] || { echo 'TLS_PRIMARY_DOMAIN must equal PORTAL_DOMAIN.' >&2; exit 1; }
