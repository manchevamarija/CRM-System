...
cat "$UPLOAD_ARCHIVE" | docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" exec -T api sh -c 'mkdir -p "${UPLOADS_ROOT:-/var/lib/crm-system/uploads}" && tar -C "${UPLOADS_ROOT:-/var/lib/crm-system/uploads}" -xzf -'
