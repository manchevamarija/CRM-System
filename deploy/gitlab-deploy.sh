...
ARCHIVE="crm-system-${RELEASE_ID}.tar.gz"
...
ssh "$REMOTE" "set -eu; tar -xzf '/tmp/$ARCHIVE' -C '$REMOTE_RELEASE'; rm -f '/tmp/$ARCHIVE'; ln -sfn '$REMOTE_RELEASE' '$DEPLOY_PATH/current'; cd '$DEPLOY_PATH/current'; docker compose -p crm-system --env-file '$DEPLOY_PATH/shared/.env.production' -f docker-compose.production.yml up -d --build --remove-orphans; docker compose -p crm-system --env-file '$DEPLOY_PATH/shared/.env.production' -f docker-compose.production.yml ps"
