...
BACKUP_ROOT="${BACKUP_ROOT:-/var/backups/crm-system}"
...
docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" exec -T postgres sh -c 'PGPASSWORD="$POSTGRES_PASSWORD" pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Fc' > "$BACKUP_ROOT/database/crm-system-$STAMP.dump"
docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" exec -T api sh -c 'root="${UPLOADS_ROOT:-/var/lib/crm-system/uploads}"; mkdir -p "$root"; tar -C "$root" -czf - .' > "$BACKUP_ROOT/uploads/uploads-$STAMP.tar.gz"
