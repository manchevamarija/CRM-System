# CRM System — One Stop Shop Portal — техничко предавање

## Преглед на проектот

CRM System — One Stop Shop Portal е веб-платформа наменета за:
...
### Production domain and affiliate paths
Current production target is `https://crm.digitmak.mk`. DIGITMAK is the active tenant for the deployment. Affiliate partners use paths on the same host: `/digitmak`, `/hpc`, `/vezilka`, `/bau`. Affiliate attribution is stored separately from tenant ownership.

Each brand can have its own specific partners under it, using `/{brand}/{partner}` — e.g.
`/digitmak/acme-agency`, `/vezilka/some-reseller`. The brand segment must be one of the
known tenants (checked against the tenant directory); the partner segment is open-ended —
any new partner slug (lowercase letters, digits, hyphens) works immediately with no code
change. Both the brand-level and per-partner numbers are visible in the admin's Affiliate
tab, broken down separately.


## Affiliate hierarchy finalization
- Shared production host: https://crm.digitmak.mk
- Partner links: /<brand>/<partner>
- BAU initial slots: partner-01 through partner-08, created idempotently by DatabaseInitializer.
- Affiliate click attribution is bound to both brand and partner code.


## Strict affiliate attribution

Google Form submissions do not contain the partner slug because the form intentionally has no Partner field. The webhook therefore records an explicit attribution state and never guesses: `Attributed`, `BrandAttributed`, `Ambiguous`, or `Unattributed`. Only an unambiguous matched click receives conversion credit; ambiguous/unattributed leads remain visible in CRM without affiliate credit.
